...
const createTxOuts = (receiverAddress: string, myAddress: string, amount, leftOverAmount: number, accountFrom, accountTo) => {
    const txOut1: TxOut = new TxOut(receiverAddress, amount, accountFrom, accountTo);
    if (leftOverAmount === 0) {
        return [txOut1];
    } else {
        const leftOverTx = new TxOut(myAddress, leftOverAmount, accountFrom, accountTo);
...
const createTransaction = (receiverAddress: string, amount: number, privateKey: string,
                           unspentTxOuts: UnspentTxOut[], txPool: Transaction[], accountFrom, accountTo): Transaction => {
    ...
    tx.txOuts = createTxOuts(receiverAddress, myAddress, amount, leftOverAmount, accountFrom, accountTo);
