...
const getAdjustedDifficulty = (latestBlock: Block, aBlockchain: Block[]) => {
    ...
    if (timeTaken < timeExpected / 2) {
        ...
        if (prevAdjustmentBlock.difficulty > 1)
            return prevAdjustmentBlock.difficulty - 1;
...
const sendTransaction = (address: string, amount: number, accountFrom, accountTo): Transaction => {
    const tx: Transaction = createTransaction(address, amount, getPrivateFromWallet(), getUnspentTxOuts(), getTransactionPool(), accountFrom, accountTo);
